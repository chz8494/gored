...
func (e *Bitbns) GetCoinsData() error {
	...
	strRequestPath := "/order/getTickerWithVolume/" // "/api/trade/v1/tickers"
	strUrl := "https://bitbns.com" + strRequestPath
	...
	for symbol, _ := range coinsData {
		if symbol != "USDT" && strings.Contains(symbol, "USDT") {
			// continue
			symbol = "INR"
		}
...
func (e *Bitbns) GetPairsData() error {
	...
	for symbol, _ := range pairsData {
		...
		if symbol != "USDT" && strings.Contains(symbol, "USDT") { // USDT pairs
			baseSymbol = "USDT"
			targetSymbol = symbol[:len(symbol)-4]
		} else {
			baseSymbol = "INR"
			targetSymbol = symbol
			symbol = targetSymbol + baseSymbol
		}
...
func (e *Bitbns) OrderBook(p *pair.Pair) (*exchange.Maker, error) {
	if p.Base.Code == "USDT" {
		return nil, fmt.Errorf("%s Orderbook Only Support inr based pair, %+v", e.GetName(), p)
	}
	...
	if err := json.Unmarshal(jsonResponse.Data, &buyOrderBook); err != nil {
	...
	if err := json.Unmarshal(jsonResponse.Data, &sellOrderBook); err != nil {
		return nil, fmt.Errorf("%s Get Orderbook Result Unmarshal Err: %v %s", e.GetName(), err, jsonResponse.Data)
	}
