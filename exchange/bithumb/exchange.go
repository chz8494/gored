...
func CreateBithumb(config *exchange.Config) *Bithumb {
	once.Do(func() {
		instance = &Bithumb{
			ID:      DEFAULT_ID,
			Name:    "Bithumb",
			Website: "https://www.bithumb.pro/en-us",
...
func (e *Bithumb) GetTradingWebURL(pair *pair.Pair) string {
	return fmt.Sprintf("https://www.bithumb.pro/en-us/spot/trade;symbol=%s_%s", e.GetSymbolByCoin(pair.Target), e.GetSymbolByCoin(pair.Base))
}
