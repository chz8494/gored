...
func CreateTradeogre(config *exchange.Config) *Tradeogre {
	once.Do(func() {
		instance = &Tradeogre{
			ID:      DEFAULT_ID,
			Name:    "Tradeogre",
			Website: "https://www.tradeogre.com/",
...
func (e *Tradeogre) GetTradingWebURL(pair *pair.Pair) string {
	return fmt.Sprintf("https://tradeogre.com/exchange/%s-%s", e.GetSymbolByCoin(pair.Base), e.GetSymbolByCoin(pair.Target))
}
